...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Piano Video → MIDI 2.1")
    ...
    def build_ui(self):
        ...
        header.pack(fill="x")
        ctk.CTkLabel(header, text="🎹  Piano Video → MIDI 2.1", font=("Arial", 28, "bold")).pack(side="left", padx=25, pady=18)
        ctk.CTkLabel(header, text="Falling Notes Converter • Pixel rulers", font=("Arial", 13)).pack(side="left", padx=5, pady=18)
    ...
    def select_video(self):
        ...
        try:
            ...
            self.status.configure(text="Video caricato. Usa gli assi X/Y per leggere le coordinate in pixel.")
